        ...
        for (int x = 0; x < mapHelper.mapData.width; x++)
        {
            for (int y = 0; y < mapHelper.mapData.height; y++)
            {
                ushort tile1SW = level1.HasTile(new Vector3Int(2 * x, 2 * y))
                    ? (ushort)((IndexedTile)level1.GetTile(new Vector3Int(2 * x, 2 * y))).index : (ushort)0;
                ushort tile2SW = level2.HasTile(new Vector3Int(2 * x, 2 *  y))
                    ? (ushort)((IndexedTile)level2.GetTile(new Vector3Int(2 * x, 2 * y))).index : (ushort)0;
                ushort tile3SW = level3.HasTile(new Vector3Int(2 * x, 2 * y))
                    ? (ushort)((IndexedTile)level3.GetTile(new Vector3Int(2 * x, 2 * y))).index : (ushort)0;
                ushort tile1SE = level1.HasTile(new Vector3Int(2 * x + 1, 2 * y))
                    ? (ushort)((IndexedTile)level1.GetTile(new Vector3Int(2 * x + 1, 2 * y))).index : (ushort)0;
                ushort tile2SE = level2.HasTile(new Vector3Int(2 * x + 1, 2 * y))
                    ? (ushort)((IndexedTile)level2.GetTile(new Vector3Int(2 * x + 1, 2 * y))).index : (ushort)0;
                ushort tile3SE = level3.HasTile(new Vector3Int(2 * x + 1, 2 * y))
                    ? (ushort)((IndexedTile)level3.GetTile(new Vector3Int(2 * x + 1, 2 * y))).index : (ushort)0;
                ushort tile1NW = level1.HasTile(new Vector3Int(2 * x, 2 * y + 1))
                    ? (ushort)((IndexedTile)level1.GetTile(new Vector3Int(2 * x, 2 * y + 1))).index : (ushort)0;
                ushort tile2NW = level2.HasTile(new Vector3Int(2 * x, 2 * y + 1))
                    ? (ushort)((IndexedTile)level2.GetTile(new Vector3Int(2 * x, 2 * y + 1))).index : (ushort)0;
                ushort tile3NW = level3.HasTile(new Vector3Int(2 * x, 2 * y + 1))
                    ? (ushort)((IndexedTile)level3.GetTile(new Vector3Int(2 * x, 2 * y + 1))).index : (ushort)0;
                ushort tile1NE = level1.HasTile(new Vector3Int(2 * x + 1, 2 * y + 1))
                    ? (ushort)((IndexedTile)level1.GetTile(new Vector3Int(2 * x + 1, 2 * y + 1))).index : (ushort)0;
                ushort tile2NE = level2.HasTile(new Vector3Int(2 * x + 1, 2 * y + 1))
                    ? (ushort)((IndexedTile)level2.GetTile(new Vector3Int(2 * x + 1, 2 * y + 1))).index : (ushort)0;
                ushort tile3NE = level3.HasTile(new Vector3Int(2 * x + 1, 2 * y + 1))
                    ? (ushort)((IndexedTile)level3.GetTile(new Vector3Int(2 * x + 1, 2 * y + 1))).index : (ushort)0;
