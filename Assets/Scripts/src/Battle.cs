...
public partial class Battle : MonoBehaviour
{
    ...
    {
        if (GetEffectiveType(move, attacker) is Type.Stellar) return defender.pokemon.terastalized ? 2.0F : 1.0F;
    ...
    private int DamageCalc(BattlePokemon attacker, BattlePokemon defender, MoveID move,
        bool isMultiTarget, bool isCritical, int side, int powerOverride = NullInt, CalcAIMode aiMode = 0)
    {
        int roll = aiMode switch 
        {
            CalcAIMode.Floor => 85,
            CalcAIMode.Ceiling => 100,
            _ => 100 - (random.Next() & 15)
        };
        ...
        switch (move.Data().effect)
        {
            ...
            case TrumpCard:
                ...
                break;
            case TeraBlast:
                effectivePower = (attacker.pokemon.terastalized && attacker.pokemon.teraType is Type.Stellar) ? 100 : 80;
                break;
            case ZMove:
            ...
            case MistyExplosion when terrain is Terrain.Misty:
                ...
                break;
            case LastRespects:
                effectivePower *= (attacker < 3 ? opponentFaints : playerFaints) + 1;
                break;
            case NaturalGift:
        ...
        */
        ...
        if (defender.vulnerable) result <<= 1;
        Debug.Log(result);
        return Max(1, result);
    }
    ...
    private bool TryToHit(int attacker, int defender, MoveID move)
    {
        ...
        if (GetMove(attacker).accuracy == Move.AlwaysHit || PokemonOnField[defender].vulnerable
            || (Moves[attacker] is MoveID.Toxic && PokemonOnField[attacker].HasType(Type.Poison))
            || move.Data().moveFlags.HasFlag(alwaysHitsInRain) && IsRainAffected(defender)
            || move.Data().moveFlags.HasFlag(alwaysHitsInSnow) && IsWeatherAffected(defender, Snow)
            || (move.Data().moveFlags.HasFlag(alwaysHitsMinimized) && PokemonOnField[defender].minimized)
            || HasAbility(attacker, NoGuard)
            || HasAbility(defender, NoGuard)
            || (PokemonOnField[attacker].hasMindReader && PokemonOnField[attacker].mindReaderTarget == defender)
            || GetAccuracy(move, attacker, defender) > random.NextDouble())
        {
            PokemonOnField[defender].isHit = true;
            return true;
        }
    ...
    private IEnumerator ExecuteMove(int index, bool parentalBond = false, bool dancer = false)
    {
        ...
        user.vulnerable = GetMove(index).effect is GlaiveRush;
        ...
        switch (GetMove(index).effect)
        {
            ...
            case MoveEffect.Pledge when AlliesWillPledge(index):
                ...
                user.done = true;
                MoveCleanup();
                yield return CheckLeppaBerry(index, MoveNums[index]);
                yield break;
            case MirrorMove:
            ...
            case Instruct:
                ...
                BattlePokemon mon = PokemonOnField[target];
                if (mon.GetMoveSlot(mon.lastMoveUsed) is NullInt)
                {
                    yield return Announce(BattleText.MoveFailed);
                    user.moveFailedThisTurn = true;
                    user.done = true;
                ...
                bool storedDone = mon.done;
                MoveID storedMove = Moves[target];
                int storedSlot = MoveNums[target];
                Moves[target] = mon.lastMoveUsed;
                MoveNums[target] = mon.lastMoveSlot;
                mon.done = false;
                yield return TryMove(Targets[index]);
        ...
        switch (GetMove(index).effect)
        {
            ...
            case RevivalBlessing:
                ...
                {
                    bool goAhead = false;
                    foreach (Pokemon mon in playerPokemon)
                    {
                        if (mon.exists && mon.fainted) goAhead = true;
                    }
                    if (!goAhead)
                    {
                        yield return Announce(BattleText.MoveFailed);
                    ...
                    else
                    {
                        choseSwitchMon = false;
                        menuManager.currentPartyMon = 1;
                        menuManager.PartyMenu(MenuManager.PartyScreenReason.RevivalBlessing);
                        state = BattleState.PlayerInput;
                        while (!choseSwitchMon)
                        {
                            yield return null;
                        }
                        Pokemon mon = playerPokemon[switchingTarget];
                        mon.fainted = false;
                        mon.hp = mon.hpMax >> 1;
                        yield return Announce(mon.MonName + " was revived!");
                    }
                }
                else
                {
                    int whichMon = GetRevivedMonSingle();
                    if (whichMon is NoMons)
                    {
                        yield return Announce(BattleText.MoveFailed);
                        user.moveFailedThisTurn = true;
                    }
                    ...
                    {
                        Pokemon mon = opponentPokemon[whichMon];
                        mon.fainted = false;
                        mon.hp = mon.hpMax >> 1;
                        yield return Announce(mon.MonName + " was revived!");
                    }
                ...
                or DoubleIronBash or DragonDarts or ScaleShot or MultiHit3:
                {
                    if (hitAnyone)
                    {
                        ...
                        for (int i = 0; i < hits; i++)
                        {
                            ...
                            if (GetMove(index).effect is TripleHit or PopulationBomb
        ...
        switch (GetMove(index).effect)
        {
            ...
            case Uproar:
                ...
                if (user.uproarTimer == 3)
                {
                    ...
                    + " is making an uproar!");
                break;
            case Crash50Max or AxeKick when user.moveDamageDone == 0 &&
        ...
        if (!dancer && Moves[index].Data().moveFlags.HasFlag(danceMove))
        {
            List<int> dancers = new();
            for (int i = 0; i < 6; i++)
            {
                if (HasAbility(i, Dancer) && i != index) dancers.Add(i);
            }
    ...
    private IEnumerator DoMoveEffect(int index, MoveID move, int attacker)
    {
        ...
        switch (effect)
        {
            ...
            case MoveEffect.HealBell:
                ...
                break;
            case AttackUp1:
            //case OrderUp when PokemonOnField[index].commanded && PokemonOnField[index].commandingSpecies is SpeciesID.TatsugiriCurly:
                yield return StatUp(index, Attack, 1, attacker);
                break;
            case AttackUp2:
                yield return StatUp(index, Attack, 2, attacker);
            ...
            case FlowerShield:
            case DefenseUp1:
            //case OrderUp when PokemonOnField[index].commanded && PokemonOnField[index].commandingSpecies is SpeciesID.TatsugiriDroopy:
                yield return StatUp(index, Defense, 1, attacker);
                break;
            case DiamondStorm:
                int stages = 0;
            ...
            case AuraWheel:
            case SpeedUp1:
            //case OrderUp when PokemonOnField[index].commanded && PokemonOnField[index].commandingSpecies is SpeciesID.TatsugiriStretchy:
                yield return StatUp(index, Speed, 1, attacker);
                break;
            case SpeedUp2:
            ...
            case AttackDefenseDown1:
                ...
                yield return StatDown(index, Defense, 1, index);
                break;
            case TeraBlast when PokemonOnField[attacker].pokemon.terastalized
                && PokemonOnField[attacker].pokemon.teraType is Type.Stellar:
            ...
            case SwitchHit:
                ...
                {
                    int nextMon = GetNextOpponentMonSingle(); //Todo: Doubles
                    if (nextMon == NoMons)
                    {
            ...
            case Memento:
                yield return StatDown(index, Attack, 2, attacker);
                yield return StatDown(index, SpAtk, 2, attacker);
                yield return DoFatalDamage(attacker);
            ...
            case SilkTrap:
                yield return AnnounceProtect(index);
                PokemonOnField[index].protection = Protection.SilkTrap;
                break;
            case MatBlock:
                yield return Announce(MonNameWithPrefix(index, true) +
            ...
            case ContinuousDamage:
                ...
                break;
            case SaltCure:
                PokemonOnField[index].saltCure = true;
                yield return Announce(MonNameWithPrefix(index, true) + " is being salt cured!");
                break;
            case StealthRock:
    ...
    private IEnumerator DoProtectEffect(int protector, int attacker)
    {
        switch (PokemonOnField[protector].protection)
        {
            ...
            case Protection.Obstruct:
                ...
                break;
            case Protection.SilkTrap:
                doStatAnim = true;
                yield return StatDown(attacker, Speed, 1, protector);
                break;
            default: yield break;
        }
    }
    ...
    private IEnumerator EndTurn()
    {
        ...
        IEnumerator CheckSaltCure(int index)
        {
            BattlePokemon mon = PokemonOnField[index];
            if (mon.saltCure)
            {
                yield return DoDamage(mon, 
                    mon.HPMax >> (mon.HasType(Type.Water) || mon.HasType(Type.Steel) ? 2 : 3), 
                    true, true);
                yield return Announce (MonNameWithPrefix(index, true) + " is hurt by Salt Cure!");
                yield return ProcessFaintingSingle(index);
                yield return ProcessBerries(index, false);
                yield return CheckWimpOut();
            }
        }
        yield return DoMonChecks(CheckSaltCure);
    ...
    private IEnumerator StartTurn()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (currentMon.exists)
            {
                ...
                if (currentMon.invulnerability is Invulnerability.Swallowed)
                {
    ...
    private int GetNextOpponentMonSingle() //Todo: AI
    {
        for (int i = 0; i < 6; i++)
        {
    ...
    private int GetRevivedMonSingle() //Todo: AI
    {
        for (int i = 0; i < 6; i++)
        {
            if (opponentPokemon[i].exists == true && opponentPokemon[i].fainted)
            {
                return i;
            }
        }
    ...
    public bool CanUseZMove(int index, int moveSlot)
    {
        ...
        if (ZMoveLockedIn(index > 2)) return false;
        Pokemon mon = PokemonOnField[index].pokemon;
